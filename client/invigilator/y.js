...
<body id="page-top">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-12 col-md-6 col-lg-6 form-group">
                         <select class="form-control" v-model="selected_program">
                             ...
                             :value="{key:key,program_id:program._id,level: program.level}"><span>{{program.program_name}}</span><span>   {{program.level}}</span></option>
                         </select>
                         <span class="text-danger">{{error2}}</span>
                    </div>
                    <div class="col-sm-12 col-md-6 col-lg-6 form-group">
                         <select class="form-control" v-model="selected_course_code">
                             <option value="" selected disabled>select course code</option>
                             <option v-for="(course,key) of courses" :key="key" :value="course.course_code">{{course.course_code}}</option>
                    <div class="col-sm-12 col-md-4 col-lg-4 form-group">
                         <select class="form-control" v-model="selected_program">
                             <option value="" selected disabled>select program</option>
                             <option v-for="(program,key) of programs" :key="key"
                             :value="{key:key,program_id:program._id,level: program.level}"><span>{{programs.program_name}}</span><span>{{programs.level}}</span></option>
                         </select>
                         <span class="text-danger">{{error2}}</span>
                    </div>
                    <div class="col-sm-12 col-md-4 col-lg-4 form-group">
                         <select class="form-control" v-model="selected_course_code">
                             <option value="" selected disabled>select course code</option>
                             <option v-for="(course,key) of courses" :key="key">{{course.course_code}}</option>
                         </select>
            ...
            <div class="container mb-3 col-sm-12 col-md-10 col-lg-6">
              <div class="card card-register mx-auto mt-5">
                ...
                <div class="card-body">
                  ...
                  <a class="btn btn-success btn-block" href="#" @click.prevent="view_time_table" id="attendance_modal" data-toggle="modal"
                             ...
                             <div class="modal-body" id="element-to-print">
                                 <h5 class="modal-title text-dark text-center" id="ModalLabel">Attendance List</h5>
                                     <div class="table-responsive">
                                         <table
                                             class="table table-hover table-responsive-lg table-responsive-md table-responsive-sm table-responsive-xl" id="mytable">
                                             ...
                                             <tbody>
                                                 <tr v-for="(student,key) of student_attendance" :key="key">
                                                     ...
                                                     <td>{{student.student_signature}}</td>
                                                 </tr>
                                             </tbody>
                                         </table>
                                 </div>
  ...
  const vm = new Vue({
    ...
    methods: {
      ...
      view_time_table(){
        console.log(vm.selected_course_code)
        if(!vm.selected_course_code) vm.error1 = "select course code"
        else vm.error1 = ""
        if(!vm.selected_program) vm.error2 = "select program"
        else vm.error2 = ""

        if(vm.selected_course_code.length !== 0 && vm.selected_program.length !== 0){
          vm.loading = true
          setTimeout(async () => {
              try {
                let searched_attendance = await $.ajax({
                url: "/student_attendance/search_attendance",
                method: "POST",
                data: {
                  course_code: vm.selected_course_code,
              ...
              let { res , data } = searched_attendance

              if(res === "found"){
                vm.student_attendance = data
              }
            } catch (e) {
              console.log(e)
            }
    ...
    computed: {
      ...
      courses(){
        let index = vm.selected_program.key
        vm.level = vm.selected_program.level

        let prog = vm.programs[index]
        if(prog === undefined) return null
        else return vm.programs[index].courses_offered
        

        console.log(vm.programs[index])
        return vm.programs[index]
    ...
    async created(){
      try {
        ...
        if(all_courses.data.length > 0) vm.all_courses = all_courses.data
      } catch (e) {
        console.log(e)
      }
